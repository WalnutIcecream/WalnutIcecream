# Walnut Icecream

I build things around Linux, networking, cybersecurity, backend systems,
and developer tooling. Most of my projects are either tools I wanted
for myself or experiments to understand how something works under the hood.

Currently messing with Rust, Linux internals, distributed systems, and
security.

### Projects

- [Nomad](https://github.com/WalnutIcecream/Nomad.git) — Distributed Minecraft hosting
- [Promptless](https://github.com/WalnutIcecream/Promptless.git) — Natural language interface for Bash/Zsh
- [QueryAdapter](https://github.com/WalnutIcecream/QueryAdapter.git) — Database-agnostic natural language query engine
- [cmdctl](https://github.com/WalnutIcecream/cmdctl.git) — CLI for coding-agent account management

### Links

- 🌐 [walnuticecream.lol](https://walnuticecream.lol)
- 💼 [LinkedIn](YOUR_LINKEDIN)
- 🐙 [GitHub](https://github.com/WalnutIcecream)

### Stack

`Python` `Bash` `Java` `JavaScript` `TypeScript` `C` `Rust`

`Linux` `TCP/IP` `Distributed Systems` `Docker` `Nix`
`PostgreSQL` `MongoDB` `Neo4j` `Cloudflare` `Git`
